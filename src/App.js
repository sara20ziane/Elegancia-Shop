...
const StationPrixAchat = ({ orders, showToast }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4 animate-in zoom-in-95 mt-2 md:mt-4">
      ...
      {!currentItem ? (
        <div className="flex flex-col items-center justify-center py-20 opacity-60 bg-white rounded-[2rem] border border-[#E8D5C4]/30 shadow-sm mt-4">
          <ShoppingCart size={64} className="text-[#D4B996] mb-4" />
          <h3 className="text-xl font-serif font-bold text-[#8D7B68]">Tout est commandé !</h3>
    ...
    {orderStatusesList.map((st) => (
      <option key={st} value={st}>{st}</option>
                        ...
                        <td className="p-4 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                          ...
                          <button onClick={() => setShowInvoice(o)} className="text-[#8D7B68] p-1.5 hover:bg-[#FAF7F2] rounded-lg" title="Facture"><Receipt size={16} /></button>
                        </td>
                      </tr>
                    );
                  })}
    ...
    <option key={st} value={st} style={{ direction: "ltr" }}>{st}</option>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-100">
                      <div className="flex gap-1">
                        ...
                        <button onClick={() => setShowInvoice(o)} className="text-[#8D7B68] p-1.5" title="Facture"><Receipt size={16} /></button>
                      </div>
                      <div className="flex gap-1">
        ...
        <form onSubmit={handleSaveCustomer} className="flex-1 flex flex-col min-h-0">
          <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pb-4 pr-1">
            ...
            <div className="space-y-1 mt-3">
              ...
              <input name="profileName" defaultValue={editingCustomer?.profileName || ""} placeholder="Ex: 𝓔𝓵𝓮𝓰𝓪𝓷𝓬𝓲𝓪 ✨" className="w-full p-3.5 rounded-xl bg-[#FAF7F2] text-sm font-bold text-[#8D7B68] outline-none border border-transparent focus:border-[#E8D5C4]" />
            </div>
